...
const courses = [
  {
    ...
    price: '2,000', badge: 'Most Popular',
  },
  {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {courses.map((c, i) => (
            <div
              key={i}
              className="rounded-xl border border-[#E2E8F0] overflow-hidden shadow-sm hover:shadow-lg hover:scale-[1.02] transition-all duration-300 group flex flex-col"
            >
              {/* Top banner */}
              <div className="relative bg-gradient-to-br from-[#1B3A5C] to-[#1B4332] py-8 flex items-center justify-center">
                ...
                {c.badge && (
                  <span className="absolute top-3 right-3 px-2.5 py-0.5 rounded-full bg-[#00B4D8] text-white text-xs font-semibold">
                    {c.badge}
                  </span>
                )}
              </div>
              ...
              <div className="p-5 flex flex-col flex-1">
                ...
                <p className="text-2xl font-extrabold text-[#00B4D8] mb-4">
                  PKR {c.price}<span className="text-sm font-normal text-[#4A5568]">/month</span>
                </p>
