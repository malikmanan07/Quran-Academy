    ...
    <section id="pricing" className="py-16 sm:py-24 bg-[#F0F4F8] scroll-mt-16">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12 sm:mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-[#1A1A2E] mb-3">Simple, Transparent Pricing</h2>
          <p className="text-[#4A5568]">Choose the plan that works for you</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 items-stretch">
          {plans.map((p, i) => (
            <div key={i} className={`rounded-2xl overflow-hidden flex flex-col transition-all duration-300 ${
              p.popular
              ...
              {p.popular && (
                <div className="bg-[#00B4D8] text-white text-center text-xs font-bold py-2 uppercase tracking-wider">Most Popular</div>
              )}
